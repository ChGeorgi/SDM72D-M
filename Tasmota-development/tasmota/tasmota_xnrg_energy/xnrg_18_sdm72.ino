...
const uint16_t sdm72_register[] {
  ...
  #endif  //  SDM72_IMPEXP
  #ifdef SDM72_RESETTABLE
  0x0180,   // 6 SDM72D_RESETTABLE_TOTAL_ENERGY      [kWh]
  0x0184,   // 7 SDM72D_RESETTABLE_IMPORT_ENERGY    [kWh]
  0x0186,   // 8  SDM72D_RESETTABLE_EXPORT_ENERGY     [kWh]
#endif  //  SDM72_RESETTABLE
};
...
struct SDM72 {
  ...
  float resettable_import_energy = 0;
  float resettable_export_energy = 0;
    ...
    if (error) {
      ...
      switch(Sdm72.read_state) {
        ...
        case 5:
		  ...
		  #ifdef SDM72_RESETTABLE  
		case 6:
          Sdm72.resettable_total_energy = value;    // kWh
          break;
		  
		case 7:
          Sdm72.resettable_import_energy = value;    // kWh
          break;
		  
		case 8:
          Sdm72.resettable_export_energy= value;    // kWh
          break;
...
#if defined SDM72_IMPEXP && defined SDM72_RESETTABLE
...
const char HTTP_ENERGY_SDM72[] PROGMEM =
 ...
 // "{s}" D_RESETTABLE_TOTAL_ACTIVE "{m}%s " D_UNIT_KILOWATTHOUR "{e}"
  "{s}" D_RESETTABLE_IMPORT_ACTIVE "{m}%s " D_UNIT_KILOWATTHOUR "{e}"
  "{s}" D_RESETTABLE_EXPORT_ACTIVE "{m}%s " D_UNIT_KILOWATTHOUR "{e}";
...
void Sdm72Show(bool json) {
  if (isnan(Sdm72.total_active)) { return; }
   ...
   char value2_chr[GUISZ];
    ...
    char value5_chr[GUISZ];

  if (json) {
     ResponseAppend_P(PSTR(",\"" D_JSON_EXPORT_POWER "\":%s,\"" D_JSON_IMPORT_POWER  "\":%s,\"" D_JSON_RESETTABLE_IMPORT_ACTIVE  "\":%s,\"" D_JSON_RESETTABLE_EXPORT_ACTIVE  "\":%s"),
	// \"" D_JSON_RESETTABLE_TOTAL_ACTIVE  "\":%s,
      ...
      //EnergyFormat(value3_chr, &Sdm72.resettable_total_energy, Settings->flag2.energy_resolution),
	  EnergyFormat(value4_chr, &Sdm72.resettable_import_energy, Settings->flag2.energy_resolution),
	  EnergyFormat(value5_chr, &Sdm72.resettable_export_energy, Settings->flag2.energy_resolution));
	  

#ifdef USE_WEBSERVER
  } else {
    WSContentSend_PD(HTTP_ENERGY_SDM72,  
	  WebEnergyFormat(value_chr, &Sdm72.export_power, Settings->flag2.wattage_resolution),
      WebEnergyFormat(value2_chr, &Sdm72.import_power, Settings->flag2.wattage_resolution),
      //WebEnergyFormat(value3_chr, &Sdm72.resettable_total_energy, Settings->flag2.energy_resolution),
	  WebEnergyFormat(value4_chr, &Sdm72.resettable_import_energy, Settings->flag2.energy_resolution),
	  WebEnergyFormat(value5_chr, &Sdm72.resettable_export_energy, Settings->flag2.energy_resolution));

#endif  // USE_WEBSERVER
  }
}

#elif defined SDM72_IMPEXP 
...
void Sdm72Show(bool json) {
  ...
  if (json) {
     ResponseAppend_P(PSTR(",\"" D_JSON_RESETTABLE_TOTAL_ACTIVE  "\":%s,\"" D_JSON_RESETTABLE_EXPORT_ACTIVE  "\":%s,\"" D_JSON_RESETTABLE_EXPORT_ACTIVE  "\":%s"),

      EnergyFormat(value_chr, &Sdm72.resettable_total_energy, Settings->flag2.energy_resolution),
	  EnergyFormat(value2_chr, &Sdm72.resettable_import_energy, Settings->flag2.energy_resolution),
	  EnergyFormat(value3_chr, &Sdm72.resettable_export_energy, Settings->flag2.energy_resolution));
	  

#ifdef USE_WEBSERVER
  } else {
    WSContentSend_PD(HTTP_ENERGY_SDM72, WebEnergyFormat(value_chr, &Sdm72.resettable_total_energy, Settings->flag2.energy_resolution),
	                                    WebEnergyFormat(value2_chr, &Sdm72.resettable_import_energy, Settings->flag2.energy_resolution),
	                                    WebEnergyFormat(value3_chr, &Sdm72.resettable_export_energy, Settings->flag2.energy_resolution));
